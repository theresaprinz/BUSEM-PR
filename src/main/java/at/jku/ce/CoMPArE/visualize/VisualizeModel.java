...
//            // LogHelper.logDebug("adding information for Message "+m.toString());
            ...
            if (p.getSenderOfMessage(m).getExpectedMessages().contains(m))
                ...
                for (State sendState : p.getSenderOfMessage(m).getSendStates(m))
                    senders.add(senderNode.getNode(sendState.getUUID().toString()));
            }
            ...
            if (p.getRecipientOfMessage(m).getProvidedMessages().contains(m))
                ...
                for (State recvState : p.getRecipientOfMessage(m).getRecvStates(m))
                    recipients.add(recipientNode.getNode(recvState.getUUID().toString()));
            }
...
//            // LogHelper.logDebug("adding information for Message "+m.toString());
            ...
            if (p.getSenderOfMessage(m).getExpectedMessages().contains(m))
                ...
                for (State sendState : p.getSenderOfMessage(m).getSendStates(m))
                    senders.add(graph.getNode(sendState.getUUID().toString()));
            }
            Set<Graph.Node> recipients = new HashSet<>();
            if (p.getRecipientOfMessage(m).getProvidedMessages().contains(m))
                ...
                for (State recvState : p.getRecipientOfMessage(m).getRecvStates(m))
                    recipients.add(graph.getNode(recvState.getUUID().toString()));
            }
