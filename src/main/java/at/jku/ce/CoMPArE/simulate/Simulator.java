...
public class Simulator {
    ...
    public String getPathToState(State targetState) {
        ...
        for (SimulatorStep s : overallSteps) {
            if (s.condition != null && !(s.condition instanceof MessageCondition))
                path.append(s.subject + ": select \"" + s.condition + "\" -><br>");
            ...
            for (int i = steps.size() - 1; i >= 0; i--) {
                ...
                if (s.state instanceof RecvState) {
                    ...
                    for (SimulatorStep ss : steps) {
                        if (ss.condition instanceof MessageCondition)
                            requiredMessages.add(instance.getProcess().getMessageByUUID(((MessageCondition) ss.condition).getMessage()));
                    }
                    ...
                    for (Message m : messages) {
                        if (messages.size() > 1 && requiredMessageIsContained && !requiredMessages.contains(m))
                            continue;
