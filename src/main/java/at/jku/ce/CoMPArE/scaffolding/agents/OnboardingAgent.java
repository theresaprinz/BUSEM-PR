        ...
        if (stage == OnboardingAgent.INTRO_ENACTMENT && currentInstance != null) {
            ...
            if (!alreadyActive && stepCounter == 1) {
                ...
                if (isReceiveState && !receiveShown && !alreadyActive) {
                    ...
                    if (!sendShown)
                        pointerToSendStates = "As soon as we encounter such a step, we will have a closer look.";
                    ConfirmDialog d = ConfirmDialog
                            .show(mainUI,
            ...
            if (mainUI.getRestart().isVisible() && !restartShown) {
                ...
                if (!decisionShown)
                    restartPrompt = "You can further explore the features of the app by restarting the enactment " +
